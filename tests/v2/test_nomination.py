...
class CandidateTestCase(BaseTestCase):
    ...
    def test_user_self_nominate(self):
        ...
        self.assertEqual(
            response_content['message'], "Request successfully submitted!")

    def test_nomination_no_office_candidate(self):
        """ Test that endpoint can create office"""
        super().create_user(user)
        login = super().login_user(admin_user_login)
        login_content = json.loads(login.data.decode('utf-8'))
    ...
    def test_nominate_blank_user(self):
        """ Test that endpoint can create office"""
        super().create_user(user)
        login = super().login_user(admin_user_login)
        login_content = json.loads(login.data.decode('utf-8'))
    ...
    def test_nominate_nonint_politician(self):
        """ Test that endpoint can create office"""
        super().create_user(user)
        login = super().login_user(admin_user_login)
        login_content = json.loads(login.data.decode('utf-8'))
    ...
    def test_nominate_existing_politician(self):
